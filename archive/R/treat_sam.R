...
lbt <- oo %>%
  group_by(day) %>%
  summarise(lab_total = sum(tested_all, na.rm = TRUE))

tst <- tst %>%
  left_join(lbt, by = "day") %>%
  mutate(tests_daily = ifelse(is.na(lab_total), daily_tests, lab_total), tested = cumsum(tests_daily))

tl <- dl %>%
  left_join(tst %>% select(day, tested)) %>%
  select(-confirmed) %>%
  mutate(confirmed = cumsum(incidence), country = "Lithuania") %>%
  select(country, day, confirmed, deaths, recovered, tested, under_observation, quarantined)

dd <- read.csv("data/lt-covid19-daily.csv") %>%
  mutate(day = ymd(day)) %>%
  arrange(day)
