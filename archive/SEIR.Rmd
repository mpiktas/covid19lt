...
dt <- dt %>%
  ...
  select(day, confirmed, incidence, Recovered = recovered, Deaths = deaths) %>%
  mutate(day = ymd(day), w = ifelse(incidence == 0, 0, 1))
dtf <- dt
dt <- dt %>%
  filter(day >= "2020-03-08") %>%
  mutate(times = 1:n())
...
one_day <- lapply(TT, function(end) {
  ...
  m1r <- mseirp1(tm, m1$par, t_N = 9, I0 = 1, N = 2793196) %>%
    data.frame() %>%
    mutate(day = fday, incidence = c(0, round(diff(conf.flow)))) %>%
    mutate(Dead = round(R_Fatal), Infectious = round(I), Exposed = round(E), confirmed = round(conf.flow), Hospital = round(Severe_H + Fatal), Recovered = round(R_Mild + R_Severe)) %>%
    select(day, time, confirmed, incidence, Infectious, Exposed, Hospital, Dead, Recovered)
...
coefs <- one_day %>%
  lapply(function(l) data.frame(end = max(l$dt$day), R = l$p[1], mu = l$p[2])) %>%
  bind_rows()
...
df <- coefs %>%
  ...
  mutate(R = round(R, 2), mu = round(mu, 2))
...
ggplot(aes(x = day, y = Re, group = end, colour = end), data = Re) +
  geom_line() +
...
zz <- fr %>%
  ...
  arrange(day) %>%
  slice(1) %>%
  select(forecast_day = end, epidemic_end = day, confirmed_total = confirmed)
...
pm1r <- codf %>%
  filter(end == max(end)) %>%
  select(day, Infectious, Exposed, Hospitalized = Hospital, Fatalities = Dead) %>%
  pivot_longer(Infectious:Fatalities, "Compartment") %>%
  mutate(Compartment = factor(Compartment, levels = c("Exposed", "Infectious", "Hospitalized", "Fatalities")))
ggplot(aes(x = day, y = value, group = Compartment), data = pm1r) +
  geom_col(aes(fill = Compartment)) +
  scale_fill_manual(values = c("Exposed" = "yellow", "Infectious" = "red", "Hospitalized" = "blue", "Fatalities" = "black"))
```
...
```{r, echo = FALSE}
ggplot(aes(x = day, y = Deaths, group = end, colour = end), data = codf %>% rename(Deaths = Dead) %>% filter(end >= max(dt$day) - days(mc$show - 1))) +
  geom_line() +
  geom_point(aes(x = day, y = Deaths, group = NULL), colour = "black", data = dt)
```
...
```{r, echo = FALSE}
ggplot(aes(x = day, y = Recovered, group = end, colour = end), data = codf %>% rename(Deaths = Dead) %>% filter(end >= max(dt$day) - days(mc$show - 1))) +
  geom_line() +
  geom_point(aes(x = day, y = Recovered, group = NULL), colour = "black", data = dt)
```

## Hospitalizuotų pacientų prognozės

```{r, echo = FALSE}
ggplot(aes(x = day, y = Hospital, group = end, colour = end), data = codf %>% rename(Deaths = Dead) %>% filter(end >= max(dt$day) - days(mc$show - 1))) +
  geom_line()
```
