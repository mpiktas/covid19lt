...
coefs <- one_day %>% lapply(function(l)data.frame(end = max(l$dt$day), r = l$p[1], p = l$p[2])) %>% bind_rows()
...
df <- coefs %>% filter(end >= max(end)- days(mc$show - 1)) %>% mutate(r = round(r, 2), p = round(r, 2))
