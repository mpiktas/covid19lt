...
lvl3 <- ss %>%
  left_join(cs %>% select(-municipality_code)) %>%
  left_join(adm %>% select(-municipality_name, -population2020) %>% rename(population = population2021)) %>%
  ...
  mutate(
    vaccinated_1 = fixNA(vaccinated_1),
...
lvl11 <- lvl1 %>%
  left_join(hosp1) %>%
  mutate(
    hospitalized = fixNA(hospitalized),
    icu = fixNA(icu),
