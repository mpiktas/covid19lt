...
if(grepl("apskr", params$region)) {
  ...
  if(params$region == "Lietuva") {
      ...
      aa <- read.csv("../data/lt-covid19-level3.csv") %>% rename(region = administrative_level_2)
...
xi <- xts(aa %>% select(confirmed = confirmed_daily, deaths = deaths_daily, tests = tests_daily, tpnew = tests_positive_new_daily), order.by = aa$day)
...
```{r}
dygraph(a714[,c("tpnew","tpnew3","tpnew7")], main = paste("Dieniniai atvejai ir slenkantys vidurkiai:",last_day)) %>% dyOptions(colors = RColorBrewer::brewer.pal(3, "Set1")[c(3,2,1)]) %>% dyRangeSelector
