...
dd <- data.frame(date = ymd("2022-01-01") + days(0:20), hotfix = 31 + 0:20)
# begin nolint
...
lap <- lapply(curdate - days(0:2), function(curd) {
  ...
  lnk <- glue::glue("https://covid19-static.cdn-apple.com/covid19-mobility-data/2204HotfixDev{hf}/v3/en-us/applemobilitytrends-{cd}.csv") # nolint
  print(lnk)
  try(read.csv(lnk))
})
