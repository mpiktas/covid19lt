...
dd <- data.frame(date = ymd("2021-05-17")+days(0:20), hotfix =13+0:20)
...
lap <- lapply(curdate - days(0:2), function(curd) {
    ...
    lnk <- glue::glue("https://covid19-static.cdn-apple.com/covid19-mobility-data/2108HotfixDev{hf}/v3/en-us/applemobilitytrends-{cd}.csv")
    print(lnk)
    try(read.csv(lnk))
})
