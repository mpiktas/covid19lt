...
dd <- data.frame(date = ymd("2021-05-27")+days(0:20), hotfix =7+0:20)
...
lap <- lapply(curdate - days(0:2), function(curd) {
    ...
    lnk <- glue::glue("https://covid19-static.cdn-apple.com/covid19-mobility-data/2109HotfixDev{hf}/v3/en-us/applemobilitytrends-{cd}.csv")
    print(lnk)
    try(read.csv(lnk))
})
