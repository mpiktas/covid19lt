...
dt0 <- aa %>% select(day, deaths_daily, other_deaths_daily) %>% mutate(d7 = rollsum(deaths_daily, 7, fill = 0, align = "right"), a7 = rollsum(deaths_daily+other_deaths_daily, 7, fill = 0, align = "right"))
...
cmp <- RR %>% select(day, deaths = `Mean(R)`) %>% inner_join(rr1 %>% select(day, cases = `Mean(R)`)) %>% inner_join(RR1 %>% select(day, all_deaths = `Mean(R)`))

xcmp <- xts(cmp %>% select(-day), order.by = cmp$day)

dygraph(xcmp) %>% dyOptions(colors = RColorBrewer::brewer.pal(3, "Set1")[c(1,2,3)]) %>% dyRangeSelector
