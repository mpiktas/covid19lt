...
osp1 <- read.csv("https://opendata.arcgis.com/datasets/ba35de03e111430f88a86f7d1f351de6_0.csv") # nolint

if (FALSE) {
  dd <- ymd(ymd_hms(unique(osp0$date)))
...
if (nrow(osp3) == nrow(osp2)) {
  osp4 <- osp3 %>%
    select(day, administrative_level_2, administrative_level_3,
      age = age_gr, sex = sex,
      confirmed_daily = incidence,
      infection_1_daily = infection_1,
      infection_2_daily = infection_2,
      infection_3_daily = infection_3,
      infection_4_daily = infection_4,
