...
dtl0 <- mapply(function(dt, tm) dt %>% mutate(downloaded = tm), lbd, pt, SIMPLIFY = FALSE) %>%
  bind_rows() %>%
  select(-day) %>%
...
oo <- dtl %>%
  select(-laboratory) %>%
  rename(laboratory = lab_actual) %>%
  group_by(day, laboratory) %>%
  summarise_all(sum) %>%
  ungroup()
...
tt <- read.csv("data/lt-covid19-total.csv") %>%
  mutate(day = ymd(day)) %>%
  mutate(incidence = ddf(confirmed), daily_tests = ddf(tested))

dd <- read.csv("data/lt-covid19-daily.csv") %>% mutate(day = ymd(day))

tt %>%
  inner_join(dd %>% select(day, di = incidence)) %>%
  mutate(d = di - incidence) %>%
  .$d
tt %>%
  inner_join(dd %>% select(day, dt = daily_tests)) %>%
  mutate(d = dt - tests_daily) %>%
  View()

tt1 <- tt %>%
  filter(day <= "2020-04-26") %>%
...
fns1 %>% lapply(function(fn) {
  dd <- read.csv(paste0("raw_data/sam/", fn)) %>%
    mutate(day = ymd(day)) %>%
    select(-country)
...
sam <- mapply(function(dt, tm) {
  dt %>%
    mutate(downloaded = tm) %>%
    select(-day)
}, samd, pt, SIMPLIFY = FALSE) %>%
  bind_rows() %>%
  mutate(day = ymd(floor_date(downloaded, unit = "day")) - days(1))
...
tl <- dl %>%
  select(-confirmed) %>%
  mutate(confirmed = cumsum(incidence)) %>%
...
zz <- tt %>%
  select(-country, -intensive_therapy, -hospitalized, -day) - tl %>%
  select(-day)

zz <- dl %>%
  filter(day >= "2020-04-18") %>%
  select(-day, -under_observation) - dd %>%
  select(-day, -country)
