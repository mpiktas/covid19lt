...
lv31 <- lv3 %>% inner_join(sav) %>% inner_join(adm %>% select(administrative_level_2, administrative_level_3))

if(nrow(lv3) ==nrow(lv31)) {
    lv32 <- lv31 %>% select(administrative_level_2, administrative_level_3, day,
...
if(nrow(lv2) ==nrow(lv21)) {
    lv22 <- lv21 %>% select(administrative_level_2, day,
                            retail_and_recreation, grocery_and_pharmacy, parks,
                            transit_stations, workplaces, residential)
