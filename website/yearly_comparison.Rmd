...
dd <- lv1 %>%
  ...
  mutate(
    ...
    hospitalizacijos = hospitalized / mx$hospitalized[1] * 100,
    rits = icu / mx$icu[1] * 100,
...
ii <- dd %>%
  select(day, rits, hospitalizacijos) %>%
  left_join(tr21v)
...
Hospitalizacijos ta pati laiko eilutė (neišskirta nei pagal amžių nei pagal vakcinacijos statusą). Nevakcinuoti tai 0-19 ir 60+ amžiaus grupės asmenų neturėjusių skiepo ir teigiamo PGR/antigeno/antikūnų testo naujų atvejų skaičiai, 14 dienų vidurkiai normalizuoti pagal maksimalią atitinkamos laiko eilutės reikšmę.
