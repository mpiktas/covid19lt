---
title: "COVID-19 vakcinacijos efektas amžiaus grupėse (7 dienų)"
...
agve <- add_states(tr, init, group = "age_gr") %>%
  mutate(
    ani1 = ifelse(sr1 < 10000, NA, ani1),
    ani2 = ifelse(sr2 < 10000, NA, ani2),
    ani3 = ifelse(sr3 < 4000, NA, ani3)
  )
...
xx <- lapply(agr, function(a) {
  ...
  xts(ss %>% select(neapsaugoti = ani0, apsaugoti2 = ani2, apsaugoti3 = ani3), order.by = ss$day)
})
names(xx) <- agr
...
dogr <- function(xx, a) {
  dygraph(xx[[a]] / 10,
    main = paste("1m 7 dienų rodiklis amžiaus grupėje ", a, sep = "")
  ) %>%
...
Šiame puslapyje rodikliai pateikiami 1 milijonui gyventojų per 7 dienas.
