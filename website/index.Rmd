...
```{r, echo = FALSE}
ii <- read.csv("../data/nvsc/lt-covid19-individual.csv") %>% mutate(day = ymd(day))
...
ll <- lv2 %>% select(day, Apskritis = administrative_level_2, Atvejai = confirmed_100k, `Teigiamų tyrimų dalis` = tpr_tpn, Mirtys = deaths_100k, Testai = tests_100k) %>% filter(Apskritis != "Unknown")
