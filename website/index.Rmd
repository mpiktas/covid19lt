...
```
  ...
  - [Vakcinacijos efektas naujiems atvejams amžiaus grupėse](case_split.html)
...
dygraph(bb, main = "Efektyvus reprodukcinis skaičius R") %>%
...
```{r}
dygraph(xaa["2020-12-27/", c("vaccinated_1_percent", "vaccinated_2_percent")], main = paste("Paskiepytų pirma ir antra doze  procentas", aa_last_day)) %>%
...
dose2 <- vv2 %>% pivot_wider(id_cols = "age", names_from = sex, values_from = "p2")
...
### Vakcinuotų dviem dozėm procentai amžiaus grupėse, `r ad_last_day`

```{r}
pm2 <- ggplot(aes(x = Amžius, y = Atvejai),
  data = dose2 %>% rename(Amžius = age, Atvejai = Moteris)
) +
  geom_col(fill = cols[1]) +
  geom_text(aes(label = Atvejai), hjust = 1.1) +
  coord_flip() +
...
pv2 <- ggplot(aes(x = Amžius, y = Atvejai),
  data = dose2 %>% rename(Amžius = age, Atvejai = Vyras)
) +
  geom_col(fill = cols[2]) +
  geom_text(aes(label = Atvejai), hjust = -0.1) +
  coord_flip() +
