...
dygraph(bb, main = "Efektyvus reprodukcinis skaičius R") %>% dySeries(c("lwr","R","upr"), label = "R") %>% dyRangeSelector(dateWindow = view_interval)
...
dygraph(a714[, c("confirmed", "confirmed7","confirmed14")], main = paste("Nauji atvejai ir slenkantys vidurkiai:",max(aa$day))) %>% dyOptions(colors = RColorBrewer::brewer.pal(3, "Set1")[c(3,2,1)]) %>% dyRangeSelector(dateWindow = view_interval)
```


```{r, echo = FALSE}

dygraph(a714["2020-03-20/",c("deaths","deaths7","deaths14")], main = paste("Mirtys per dieną ir slenkantys vidurkiai",max(aa$day))) %>%  dyOptions(colors = RColorBrewer::brewer.pal(3, "Set1")) %>% dyRangeSelector(dateWindow = view_interval)
```
...
dygraph(ttn["2020-04-01/",], main = paste("Naujų atvejų skaičius tenkantis 100 testų", max(aa$day))) %>% dyOptions(colors = RColorBrewer::brewer.pal(3, "Set1")[c(3,2,1)]) %>% dyRangeSelector(dateWindow = view_interval) 
```
...
dygraph(tpntpr["2020-04-01/"], main = paste("Teigiamų testų skaičius tenkantis 100 testų", max(aa$day))) %>% dyOptions(colors = RColorBrewer::brewer.pal(3, "Set1")[c(3,2,1)]) %>% dyRangeSelector(dateWindow = view_interval) 
```
