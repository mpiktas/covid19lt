...
data <- Sys.time()
```
  - [Savivaldybių statistika](https://covid19.myliuduomenis.lt/savivaldybes.html)    
  - [Europos šalių 14 dienų atvejai 100 tūkst. gyventojų](countries.html)
  - [Testų statistika](tests.html)
  - [Duomenys pagal savaites](weeks.html)
...
dygraph(bb, main = paste("Mirčių 7 dienų slenkantys vidurkiai",max(aa$day))) %>%  dyOptions(colors = RColorBrewer::brewer.pal(3, "Set1")[c(1,3,2)]) %>% dyRangeSelector(dateWindow = view_interval)
...
oo <- ad%>% filter(day >= ad_last_day- days(13)) %>% inner_join(agr, by = "age") %>% select(-age) %>% rename(age=age1) %>% group_by(age, sex) %>% summarise(n = sum(confirmed_daily)) %>% right_join(as_all, by = c("age", "sex")) %>% select(-n1) %>% mutate(n = fixNA(n)) %>% rename(Atvejai = n, Amžius = age) %>% pivot_wider(names_from = "sex", values_from = "Atvejai", values_fill = 0)
...
oo <- ad%>% filter(day >= ad_last_day- days(13)) %>% inner_join(agr, by = "age") %>% select(-age) %>% rename(age=age1) %>% group_by(age, sex) %>% summarise(n = sum(deaths_3_daily)) %>% right_join(as_all, by = c("age", "sex")) %>% select(-n1) %>% mutate(n = fixNA(n)) %>% rename(Atvejai = n, Amžius = age) %>% pivot_wider(names_from = "sex", values_from = "Atvejai", values_fill = 0)
...
ll <- lv2 %>% select(day, Apskritis = administrative_level_2, Atvejai = confirmed_100k, `Teigiamų tyrimų dalis` = tpr_dgn, Mirtys = deaths_100k, Testai = tests_100k) %>% filter(Apskritis != "Unknown")
