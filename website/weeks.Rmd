...
lv11 <- lv1 %>%
  select(day, tests_daily, confirmed_daily, deaths_daily, tests_mobile_daily, tpr_dgn, tpn = tests_positive_daily, hospitalized, icu, vaccinated_1_daily, vaccinated_2_daily) %>%
...
```{r}
ggplot(aes(x = wfday, y = vaccinated_1_daily + vaccinated_2_daily, group = week, colour = as.character(week)), data = lv12 %>% filter(day >= start_day)) +
  geom_line() +
  scale_colour_manual(name = "Savaitė", values = RColorBrewer::brewer.pal(5, "Set1")) +
  theme_bw() +
  labs(x = "Savaitės diena", y = "Sunaudota vakcinų", title = "Sunaudotų vakcinų skaičius savaitės dienomis")
```
