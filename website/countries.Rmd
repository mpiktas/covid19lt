...
```{r echo = FALSE}
xx <- x %>% ungroup %>% select(date, confirmed, tests,deaths, vaccines,hosp, population, country = administrative_area_level_1)
ddiff <- function(x)c(0, diff(x))
xx1 <- xx %>% filter(date!=max(date)) %>% arrange(country, date) %>%  group_by(country) %>%
    mutate(incidence = ddiff(confirmed), tests_daily = ddiff(tests),
...
xx2 <- xx1 %>% filter(country %in% c("Lithuania","Latvia","Estonia","Poland", "Germany")) %>% mutate(date = ymd(date))
...
dygraph(pivot_one(xx2,"incidence14_100k")[,cns],  main = "Cases per 100k population for the last 14 days") %>% 
    ...
    dyRangeSelector %>% dyCSS("left_legend.css")
