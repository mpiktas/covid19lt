...
x  %>% filter(date == Sys.Date() -days(2) | date == Sys.Date() - days(16)) %>% group_by(administrative_area_level_1) %>% summarize(cases14 = round(diff(confirmed)/unique(population)*1e5,2)) %>% arrange(cases14) %>% rename(country = administrative_area_level_1) %>% mutate(rank = 1:n()) %>% select(country, cases14) %>% datatable( 
