...
seir <- function(t, state, parameters) {
    with(as.list(c(state, parameters)), {
        ...
        list(c(dS, dE, dI, dMild, dSevere, dSevere_H, dFatal, dR_Mild, dR_Severe, dR_Fatal))
    })
}
...
mseir_param <- function(times, R0 = 2, mu = 0.01, D_infectious = 2.9, D_incubation = 5.2, t_N = round(times[1] + range(times)/2), I0 = 1, N = 2793196) {
    ...
    params = c(R0 = R0, mu = mu, t_N = t_N,
               p_severe = p_severe, p_fatal = death_rate, p_mild = 1 - p_severe - death_rate,
...
m1r <- mseirp1(tm, m1$par, t_N = 9, I0 = 1, N = 2793196) %>% data.frame %>% mutate(day = fday, incidence = c(0, round(-diff(S)))) %>% mutate(Dead = round(R_Fatal), Infectious = round(I), Exposed = round(E), confirmed =  round(2793196 -S), Hospital = round(Severe_H+Fatal), Recovered = round(R_Mild+R_Severe)) %>% select(day, time, confirmed, incidence, Infectious, Exposed, Hospital, Dead, Recovered)

m2r <- mseirp2(tm, m2$par, t_N = 9, I0 = 1, N = 2793196) %>% data.frame %>% mutate(day = fday, incidence = c(0, -diff(S))) %>%  mutate(Dead = round(R_Fatal), Infectious = round(I), Exposed = round(E), confirmed =  round(2793196 -S), Hospital = round(Severe_H+Fatal), Recovered = round(R_Mild+R_Severe)) %>% select(day, time, confirmed, incidence, Infectious, Exposed, Hospital, Dead, Recovered)

res <- bind_rows(m1r %>% mutate(Model = "SEIR1"), m2r %>% mutate(Model = "SEIR2"))

cf <- data.frame(Model = c("SEIR1","SEIR2"),rbind(c(m1$par,2.9, 5.2),m2$par))
