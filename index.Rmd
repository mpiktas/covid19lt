...
tta <- tt %>% group_by(day) %>% summarize(tp = sum(tests_positive_new), daily_tests = sum(tests_total)) %>% mutate(ratio = 100*tp/daily_tests)

tpd <- aa %>% select(day, confirmed) %>% left_join(tta, by = "day") %>% 
