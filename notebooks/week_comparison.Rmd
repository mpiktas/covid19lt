...
```
```{r}
ggplot(aes(x = wfday, y = tpr_tpn, group = week, colour = as.character(week)), data = lv12 %>% filter(week>47))+geom_line()+scale_colour_manual(name = "Savaitė", values = RColorBrewer::brewer.pal(5, "Set1")) + theme_bw()+ labs(x="Savaitės diena", y = "Procentai", title = "Teigiamų tyrimų dalis (7 dienų vidurkis)")
```
```{r}
ggplot(aes(x = wfday, y = tpr, group = week, colour = as.character(week)), data = lv12 %>% filter(week>47))+geom_line()+scale_colour_manual(name = "Savaitė", values = RColorBrewer::brewer.pal(5, "Set1")) + theme_bw()+ labs(x="Savaitės diena", y = "Procentai", title = "Teigiamų tyrimų dalis")
```
```{r}
ggplot(aes(x = wfday, y = tpr2, group = week, colour = as.character(week)), data = lv12 %>% filter(week>47))+geom_line()+scale_colour_manual(name = "Savaitė", values = RColorBrewer::brewer.pal(5, "Set1")) + theme_bw()+ labs(x="Savaitės diena", y = "Procentai", title = "Naujų atvejų dalis")
```
