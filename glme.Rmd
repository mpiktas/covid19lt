...
one_day <- lapply(TT, function(end) { 
    ...
    r19 <- optim(log(c(0.3,0.9, sum(dt19$incidence)/2, 0.1, sum(dt19$incidence)/2)), fit_incidence, times = dt19$times, y = dt19$incidence, w = dt19$w, n = 2)
    if(r19$convergence == 1) {
      r192 <- try(optim(r19$par, fit_incidence, times = dt19$times, y = dt19$incidence, w = dt19$w, n = 2, method = "BFGS"))
      if(inherits(r192, "try-error")) {
        r19 <- optim(r19$par, fit_incidence, times = dt19$times, y = dt19$incidence, w = dt19$w, n = 2)
      } else {
      r19 <- r192
      }
    }
    p19 <- r19$p
    fr19 <- forecast_ewaves(dt19, p19, ahead = 30, n = 2) 
...
coefs <- one_day %>% lapply(function(l)data.frame(end = max(l$dt$day),n = 2,  r = exp(l$p[1]), p = exp(l$p[2]), K = exp(l$p[3]), q = exp(l$p[4]), Ct = exp(l$p[5]))) %>% bind_rows()
